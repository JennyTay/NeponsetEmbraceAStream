...
#Average the measurements from each site each month
hab <- habitat %>% 
  group_by(site) %>% 
...
ggplot(data = habitat, mapping = aes(x =site, y = depth_cm))+
  geom_boxplot()+
  ...
  labs(y = "Maximum Depth (cm)", x = "Site")
...
ggplot(data = dat, mapping = aes(x = max7avg, y = occurrence))+
  ...
  theme(axis.text = element_text(size = 9),
        ...
        panel.background = element_blank(),
        panel.border = element_rect(color = "black", fill = NA))+
  labs(x = "Max. 7-day average (°C)", y = "Occurrence", color = "Season")+
...
ggplot(data = dat, mapping = aes(x = as.factor(occurrence), y = depth))+
  geom_boxplot()
